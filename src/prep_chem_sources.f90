...
elseif(trim(grid_type)=='polar' .or. trim(grid_type) =='lambert' .or. trim(grid_type) =='mercator') then
...
!      enddo
  ...
  if(.not.there) then
     print*,'file not found: htapLL_to_fimN.bin, takes ~10 minutes to make' 
       ...
       nprg=1 ! North Pole
       WRITE(10,'(A,I2,2F13.5)')'npoints,nprg,Lat,Lon=',npoints,nprg,icos_grid4(1,nprg),icos_grid4(2,nprg)   
...
!     WRITE(10,*)icos_edge4(1,1,lat,nprg),icos_edge4(1,1,lon,nprg),icos_edge4(1,2,lat,nprg),icos_edge4(1,2,lon,nprg) ! lat,lon from first edge
      ...
      WRITE(10,'(4F13.5)')icos_edge4(6,1,lat,nprg),icos_edge4(6,1,lon,nprg),icos_edge4(6,2,lat,nprg),icos_edge4(6,2,lon,nprg) ! lat,lon from first edge
       ...
       WRITE(10,'(A,I1,2F13.5)')'nprg,Lat,Lon=',nprg,icos_grid4(1,nprg),icos_grid4(2,nprg)   
       WRITE(10,'(A,I6)')'Nprox=',icos_nprox(nprg)
       CALL FLUSH(10)
       WRITE(10,'(6I6)')(icos_prox(i1,nprg),i1=1,icos_nprox(nprg))
       CALL FLUSH(10)
...
if( use_retro  == 1 .or.  use_edgar >   0  .or. use_fwbawb  == 1 .or. 	&
    use_gocart == 1 .or.  use_streets ==1  .or. use_seac4rs == 1  ) then   
 ...
 do isp=1,nspecies
  if(spc_alloc(1,isp) == 1) then
     ...
     if(ident > 0) then
       ...
       found_emiss_spc(iespc,bburn)   = 1
       print*,"BB=",emiss_spc_name(iespc,bburn),iespc,bbbEM_spc_name(ident),ident
     else
      !------------------------------------------------  
      call convert_inventory_to_mech_name(isp,iespc,ident,'bburn'  ,'AeM',spc_name(isp))
...
if(trim(special_output_to_wrf)=='YES' .or. trim(special_output_to_wrf)=='yes' .or. trim(grid_type)=='fim') then
!if(chemical_mechanism)=='RADM_WRF_FIM')then
#if RADM_WRF_FIM
   print *, ''//achar(27)//'[32m CONVERT TO WRF or FIM '//achar(27)//'[0m'
   call write_cptec_to_wrf_fim(wfln(3),iday,imon,iyear)
#else
   print*, ''//achar(27)//'[31m Module WRF not installed. See how to compile it in INSTALL doc '//achar(27)//'[0m' 
   stop 555
#endif
...
if (trim(chemical_mechanism) == 'RACM') then 
   ...
   return
elseif (trim(chemical_mechanism) == 'RADM_WRF_FIM') then 
