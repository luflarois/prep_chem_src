...
module bbbem_emissions    
  use AeM_emission_factors,  only: nspecies=>AeM_nspecies, N2_nitrogenio=>N2, AeM_spc_name
...
subroutine process_bbbem(iyear,imon,iday,ihour,ng,ngrids,n1,n2,n3,rlat,rlon,rland&
  ...
  ! Changes to read date and hout from the filename
  ! Gabriel, 08/03/2017
   integer dir_size,diposi,dfposi,hiposi,hfposi,nidate_abba, nitime_abba
   character  cidate_abba*7, citime_abba*4

  iunit=10
  print*,'========================================================================'
  print*,"================= processing fire emissions with 3BEM scheme ==========="
  if(ng == 1) then ! just for the first grid is needed
     ...
     if(use_bbem .eq. 1)then 
        ...
        ! DIR contains the initial letter of GOES files
        !  Ex.: /scratchin/grupos/catt-brams/shared/datain/GOES_BRFLAS/2010/001/f
        dir_size = LEN_TRIM(bbem_wfabba_data_dir)
...
! Variables idate_abba, dum(5),itime_abba
           
            cidate_abba = fnames(nfn)(diposi:dfposi)
            citime_abba = fnames(nfn)(hiposi:hfposi)
            
            READ(cidate_abba(1:7),'(i7)') nidate_abba
            READ(citime_abba(1:4),'(i4)') nitime_abba
           ...
           call read_abba(iunit,ntfocos &
                ...
                , nfocos_sensor_gt_1km,itimes(nfn),nidate_abba,nitime_abba    )
...
!                , nfocos_sensor_gt_1km,itimes(nfn)    )
    	...
    	call filter_fires_sensor_le_1km(fire_sensor_le_1km(QLONG)%fire_prop &
             , fire_sensor_le_1km(QLATI)%fire_prop &
             , fire_sensor_le_1km(QAREA)%fire_prop &
             , fire_sensor_le_1km(QSIZE)%fire_prop &
             , fire_sensor_le_1km(QFRPW)%fire_prop &
     ...
     !
     ...
     else if(use_bbem .eq. 2)then
        !---- process FRE fire product
        !
        !- look for data available for the specific day
        ! bbem_wfabba_data_dir=trim(bbem_wfabba_data_dir)!
        print*,'FRE-PIX looking for:',trim(bbem_fre_data_dir)//cyear//cjday

        call bbbem_filelist(fnames,itimes,trim(bbem_fre_data_dir)//cyear//cjday&
             ,maxfiles,nfiles)
        ...
        do nfn=1,nfiles 
           ...
           call read_fre(iunit,ntfocos &
                ...
                , fire_sensor_gt_1km(QSIZE)%fire_prop &
                , fire_sensor_gt_1km(QFRPW)%fire_prop &
                , fire_sensor_gt_1km(QFRE)%fire_prop &
		, fire_sensor_gt_1km(QTIME)%fire_prop &
                , nfocos_sensor_gt_1km    &
                , fire_sensor_gt_1km(DFRPW)%fire_prop &
                , fire_sensor_gt_1km(DSIZE)%fire_prop    )
           close(iunit)
...
110 continue
  ...
  do i=1,ic; if(qarea(i) < 1.e-6) qarea(i)=mean_area; enddo
   ...
   subroutine read_abba(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qtime,ic,itime_abba,nidate_abba,nitime_abba)
     implicit none
     !character*120 dum
     ...
     ! Changed to read from the filename instead of the header
...
110  continue
   ...
   !------------------------------------------------------------------------------------------------------
   subroutine read_fre(iunit,n,qlon,qlat,qarea,qsize,qfrpw,qfre,qtime, ic,dqfrpw,dqsize)
     ...
     integer,            intent(in)    :: iunit, &
                                          n
     integer,            intent(inout) :: ic
     real, dimension(n), intent(out)   :: qlat, &
                                          qlon, &
                                          qarea, &
                                          qsize, &
                                          qfrpw, &
...
!---------------------------------------------------------------------------------------
!MediaFRP        MW          Valor médio de FRP dos fogos detetados dentro da grade.
!DesvioFRP       MW
!MediaFsize      km2         Área média dos fogos detetados na grade.
!DesvioFsize     km2
!MediaEC         kW/m2       Valor médio da energia convectiva do fogo.
!FsizeTotal      km2         Área total dos fogos detetados na grade.
!FRE             MJ          Energia radiativa do fogo (Representa o total de biomassa
!                             consumida pelo fogo).
!NOBS            -           Número de fogos detetados dentro da grade.
!---------------------------------------------------------------------------------------
        stat = 0
     	read(iunit, '(a)') dummyString
    
     	!ic = ic+1
	!print*, 'qlon', 'qlat', 'qfrpw', 'dqfrpw', 'dqsize', 'qsize', 'qfre'
     	!do while (stat == 0)
        do i=1,n
	        ic = ic + 1
		!read(iunit, *, iostat=stat) qlon(ic),qlat(ic), qfrpw(ic), dummyReal, &

		read(iunit, *,  end=110) qlon  (ic),qlat  (ic), qfrpw(ic), dummyReal, &
					 dqfrpw(ic),dqsize(ic), dummyReal,       &
					 qsize (ic),qfre  (ic), dummyInteger
        	write(*,100)"FRE", ic, qlon(ic),qlat(ic),1.e+3*qfrpw (ic),1.e+6*qsize(ic),&
		       (1.e+3*qfrpw (ic))/(1.e+6*qsize(ic))
                100 format (1x,A3,1x,I4,1x,2F7.2,1x,2E12.3,1x,F7.3)
		! dummyReal, &
		!			    dqfrpw(ic), dqsize(ic), dummyReal,       &
		!			    qsize(ic), qfre(ic), dummyInteger
		!			    call flush(6)
...
110 continue
     	ic = ic - 1   ! numero total de focos
     print*,'=> number of accumulated fre_pix fires=',ic
     return
   end subroutine read_fre
   !------------------------------------------------------------------------------------------------------
...
555     continue
     ...
     if(use_bbem .eq. 1)then
		...
		!---  aumentando emissões 
     	...
     	do ifoc = 1,nfocos
       		...
       		if(iveg_agreg(ifoc) == 0) cycle ! eliminates false detection of fires (over ocean/water bodies/iced land)

                ! FEER COEFICIENT for TPM
                biomass_burned =  qfre(ifoc)! kg[dry biomass burned]
        	...
        	!-----
     		...
     		do iesp=1,nspecies
                    itpm=107
					! qsc(ifoc,iesp) =  biomass_burned * emission_factor(iveg_agreg(ifoc),iesp)*1.e-3 !kg[spc]
					! Changed: Measured in Kg of TPM
                    qsc(ifoc,iesp) =  biomass_burned * (emission_factor(iveg_agreg(ifoc),iesp)/emission_factor(iveg_agreg(ifoc),itpm)) !kg[spc]
                    !print*,"SPC=",AeM_spc_name(iesp),iveg(ifoc),iveg_agreg(ifoc),qsc(ifoc,iesp)
        	enddo
     	enddo
   ...
   subroutine interp_to_model(ng,n1,n2,n3,xt,yt,xm,ym,deltax,deltay,plat,plon,rlat&
        ...
        ,qfrpw,qflam,qveg,qveg_agreg, qfre, dfrpw, dsize)
     use grid_dims_out, only: grid_type,use_bbem_plumerise, use_bbem
     use bbbem_emissions
     use bbbem_plumerise
     use mem_grid, only : grid_g
     implicit none
     
     integer :: ifoc,ng,n1,n2,n3,igbox,jgbox,iesp,nspecies_actual,nfocos,i,j,icount
     real deltax,deltay,plat,plon
     !integer, dimension(nfocos) :: iveg
     real,    dimension(nfocos) :: qlon,qlat,qarea,qsize &
          ,qfrpw,qflam,qveg,qveg_agreg, qfre, dfrpw, dsize ! por foco 
     ...
     !real mtotal(3)
     real wlon,elon,xlon,splon,totalfires       !tks
     
     integer, target  :: fixedIdx, iveg_ag
     integer, pointer :: iveg_ag_ptr     
	...
	if(iveg_ag < 1 .or. iveg_ag > 4) cycle ! incompatable place for fires
        ...
        !
        !print*,'bb=',xlon,qlat(ifoc),igbox,jgbox
        if(igbox == -1 .or. jgbox == -1 ) cycle

        do iesp=1,nspecies_actual
           !- total mass emitted (flaming+smoldering) per gridbox
           bbbem_g(iesp)%src(igbox,jgbox,1) = &
	   bbbem_g(iesp)%src(igbox,jgbox,1) + qsc(ifoc,iesp)						      

           !- total mass emitted (during the flaming phase) per gridbox 	   
	   if(use_bbem_plumerise /= 1) cycle
           
	   bbbem_plume_g(iesp,iveg_ag_ptr)%src(igbox,jgbox,1) = &
	   bbbem_plume_g(iesp,iveg_ag_ptr)%src(igbox,jgbox,1) + qsc(ifoc,iesp)*flaming(iveg_ag)						  
	...
	if(use_bbem .eq. 1)then
         !-fires properties : accumulated size    
          bbbem_plume_fire_prop_g(qarea_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox)  =              &
          bbbem_plume_fire_prop_g(qarea_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox)  + qsize (ifoc)

          !-fires properties : fire number
          bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox) =              &
          bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox) + qfires (ifoc)

          if(get_STD_of_size == 1 .and.   qsize (ifoc) > 0. .and. &
             bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox) > 0.) then
             
             qarea_I_agreg( igbox,jgbox,iveg_ag,&
             nint(bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox)))&
             =qsize (ifoc)
          endif
        endif
	!------------
        !
	if(use_bbem .eq. 2)then
          !-fires properties : fire number
          bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox) =              &
          bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(igbox,jgbox) + qfires (ifoc)
	  
	  bbbem_plume_fire_prop_g(QSIZEIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  =	       &
          bbbem_plume_fire_prop_g(QSIZEIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  + qsize (ifoc)
      
	 !if( bbbem_plume_fire_prop_g(QFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)>0.)&
	 !print*,"1accum=",ifoc,igbox,jgbox,bbbem_plume_fire_prop_g(QFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox), qfrpw (ifoc)
         
	  bbbem_plume_fire_prop_g(QFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  =	       &
          bbbem_plume_fire_prop_g(QFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  + qfrpw (ifoc)
          
	 !print*,"2accum=",ifoc,igbox,jgbox,bbbem_plume_fire_prop_g(QFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox), qfrpw (ifoc)
	  
	  bbbem_plume_fire_prop_g(DFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  =	       &
          bbbem_plume_fire_prop_g(DFRPWIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  + dfrpw (ifoc)
	
          bbbem_plume_fire_prop_g(DSIZEIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  =	       &
          bbbem_plume_fire_prop_g(DSIZEIdx,iveg_ag_ptr)%fire_prop(igbox,jgbox)  + dsize (ifoc)  	  
   	endif			
     ...
     if(use_bbem_plumerise == 1) then
       ...
       if(use_bbem .eq. 1)then
        ...
        do i=1,n1
		  ...
		  ! calculate the mean size of aggregate fire
                  if( bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(i,j) .ge. 1.) then 

                     bbbem_plume_fire_prop_g(qarea_agreg ,iveg_ag_ptr)%fire_prop(i,j) = &   ! mean  size =
                       bbbem_plume_fire_prop_g(qarea_agreg ,iveg_ag_ptr)%fire_prop(i,j) / & ! total size / nfires
                       bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(i,j)
			 			  
                  else   
                     bbbem_plume_fire_prop_g(qarea_agreg ,iveg_ag_ptr)%fire_prop(i,j) = 0.
                  endif
                  ...
                  if (get_STD_of_size == 1) then
                     ...
                     do ifoc=1,nint(bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(i,j))
                       ...
                       bbbem_plume_fire_prop_g(qarea_std_agreg,iveg_ag_ptr)%fire_prop(i,j)  =  &
                            bbbem_plume_fire_prop_g(qarea_std_agreg,iveg_ag_ptr)%fire_prop(i,j)  +  &
                            ( qarea_I_agreg(i,j,iveg_ag,ifoc)                                -  & 
                            bbbem_plume_fire_prop_g(qarea_agreg ,iveg_ag_ptr)%fire_prop(i,j) )**2

                     enddo
                     !orig  qarea_std_agreg(i,j,iveg_ag) = sqrt(    qarea_std_agreg(i,j,iveg_ag)  / &
                     !orig                                        (1.e-4+qfires_agreg(i,j,iveg_ag)) )
                     bbbem_plume_fire_prop_g(qarea_std_agreg,iveg_ag_ptr)%fire_prop(i,j)  =  &
                         sqrt(bbbem_plume_fire_prop_g(qarea_std_agreg,iveg_ag_ptr)%fire_prop(i,j)) /  &
                         (1.e-4+bbbem_plume_fire_prop_g(qfires_agreg,iveg_ag_ptr)%fire_prop(i,j))
                  endif
		...
		!- convert from mass consumed during the flaming phase to fraction of the total mass consumed
                ...
                do iesp=1,nspecies_actual
                    if(bbbem_g(iesp)%src(i,j,1) > 0.) &	      
                         ...
                         bbbem_plume_g(iesp,iveg_ag_ptr)%src(i,j,1) = & 
			 bbbem_plume_g(iesp,iveg_ag_ptr)%src(i,j,1) / bbbem_g(iesp)%src(i,j,1)
                enddo
              enddo
           enddo
        enddo
       ...
       do i=1,n1
           do j=1,n2
              do iveg_ag=1,nveg_agreg
                 if(use_bbem == 2 .and. iveg_ag > 1) cycle
		 icount = 0
                 bbbem_plume_mean_g(iveg_ag_ptr)%src(i,j,1)=0.

                 do iesp=1,nspecies_actual
                    if(bbbem_g(iesp)%src(i,j,1) > 0.) then
                       icount = icount + 1
                       bbbem_plume_mean_g(iveg_ag_ptr)%src(i,j,1) = &
		       bbbem_plume_mean_g(iveg_ag_ptr)%src(i,j,1) + bbbem_plume_g(iesp,iveg_ag_ptr)%src(i,j,1)
                    endif
                 enddo
                 bbbem_plume_mean_g(iveg_ag_ptr)%src(i,j,1) = &
		 bbbem_plume_mean_g(iveg_ag_ptr)%src(i,j,1) / (float(icount)+1.e-10)
              enddo
           enddo
       enddo
     print*,"end of plumerise"
     endif
     ...
     do iesp=1,nspecies_actual
        ...
        !print*,'bb=',iesp,maxval(bbbem_g(iesp)%src(:,:,1))!,minval(grid_area(:,:)),maxval(grid_area(:,:))
     enddo
     !  
...
!        bbbem_plume_fire_prop_g(QSIZE,iveg_ag)%fire_prop(igbox,jgbox)  =              &
!             bbbem_plume_fire_prop_g(QSIZE,iveg_ag)%fire_prop(igbox,jgbox)  + qsize (ifoc)
      
!        bbbem_plume_fire_prop_g(QFRPW,iveg_ag)%fire_prop(igbox,jgbox)  =              &
!        bbbem_plume_fire_prop_g(QFRPW,iveg_ag)%fire_prop(igbox,jgbox)  + qfrpw (ifoc)
...
!_________________________ ___________________________________________________________________________      
   ...
   subroutine get_ij_rams(n1,n2,xt,yt,xm,ym,plat,plon,deltax,deltay,qlon&
     ...
     !tks      call ge_to_xy(plat,plon,stdlat2,qlon,qlat,qx,qy)   !orig
     ...
     !print*,'get_ijX',qlon, qlat,qx,qy
     !print*,'get_ijY',xt(1),xt(n1),yt(1),yt(n2)
     !
     !- 10012008
...
300  continue
   ...
   subroutine  agreg_veg(nfocos,qlat,qlon,qveg,qveg_agreg)
     ...
     do ifoc=1,nfocos
        qveg_agreg(ifoc) =catb( qveg(ifoc) )
	!print*,"veg=",ifoc,qveg(ifoc),qveg_agreg(ifoc)
     enddo
...
100  continue
   ...
   subroutine get_ij_gg(n1,n2,rlat,rlon,lon_fire,lat_fire,igbox,jgbox)
     ...
     do i=1,n1
      ...
      subroutine readOlsonHdf(nLon, nLat, olsonData)
        ...
        real,intent(out)::olsonData
        character(len=100):: filename
        character(len=5):: dsetname
        character(len=MaxChrlen):: name

        printflags=0
        filename=trim(olson_data_dir)//'.h5'
 ...
 !inicializa interface do hdf5
	call h5open_f(error)
        if(error==0)then
           !não exibe mensagem de erro do hdf5 caso ocorra um
           call h5eset_auto_f(printflags, error)
           !abre arquivo com extenção hdf5
           ...
           if(error==0)then
              ...
              if(error==0) then
                 ...
                 call h5dget_create_plist_f(dset_id,dcpl_id,error)
                 nelmts = 1
                 call H5Pget_filter_f(dcpl_id, 0, flags, nelmts,cd_values, MaxChrLen, name, filter_id, error)
                 if(error==0) then
                    !não exibe mensagem de erro do hdf5 caso ocorra um
                    call h5eset_auto_f(printflags, error)
                    !leitura da soma de fontes fixas
                    call h5dread_f(dset_id, H5T_IEEE_F32LE,olsonData,dims, error)
                    if (error == 0) then
                    ...
                    if(error==0) then
                       !não exibe mensagem de erro do hdf5 caso ocorra um
                       call h5eset_auto_f(printflags, error)					
                       !print *,dsetname,' ',maxval(olsonData)
                       !print *,dsetname,' ',minval(olsonData)				
              ...
              print*,'HDF5 file not found, please change to the correct path of the HDF5 file'
              stop
           endif
